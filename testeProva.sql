...
SELECT c.marca, c.modelo, SUM(distanciaPercorrida) as distanciaPercorrida
FROM carro c, viagem v
WHERE c.id = v.idCarro
GROUP BY c.marca, c.modelo

GO

select e.nome, SUM(v.distanciaPercorrida) as distanciaPercorrida
...
SELECT e.nome, SUM(v.distanciaPercorrida) as SomaDistanciaPercorrida , Cast(AVG(v.distanciaPercorrida)as decimal(7,2)) as MediaDist
FROM empresa e, viagem v, carro c
where e.id = c.idEmpresa and c.id = v.idCarro
group by e.nome
...
select e.nome, COUNT(c.id) as qtdCarro
...
select e.nome
from carro c right outer join empresa e on c.idEmpresa = e.id
where c.id is null

GO

select c.marca, c.modelo
from carro c left outer join viagem v on c.id = v.idCarro
where v.data is null

GO

SELECT c.modelo, COUNT(v.idCarro) as QtdViagens
